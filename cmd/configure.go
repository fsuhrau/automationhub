...
var configureCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if input == "y" {
			manager := config.Manager{
				...
				WebDriver: &config.WebDriver{
					BundleID: "com.automationhub.integration",
				},
			}
			fmt.Println("There are two different ways to create screenshots in case of test failures")
			fmt.Println("Native Screenshots are generated by OS and can capture OS Dialogs as well but be careful because it can expose sensitive parts-")
		...
		d := filepath.Join(os.Getenv("HOME"), ".automationhub/")
		os.MkdirAll(d, 0744)
		...
		configPath := filepath.Join(d, input)
		if err := os.WriteFile(configPath, y, os.ModePerm); err != nil {
			return err
		}

		fmt.Println("")
		fmt.Println("Setup finished!")
		fmt.Println("your config was written to" + configPath)
		fmt.Println("")
		fmt.Println(fmt.Sprintf("you can start the hub now via 'hub master --config %s'", configPath))

		if runtime.GOOS == "darwin" {
			fmt.Println("------------------------------------")
			input = validateInput(reader, "Would you like to create a LaunchAgent? (y/n)", []string{"y", "n"})
			if input == "y" {
				plistPath := installMacLaunchAgent(configPath)
				fmt.Println("Agent installed you can start it via:")
				fmt.Println("launchctl load " + plistPath)
			}
		}
...
func installMacLaunchAgent(configPath string) string {
	ex, _ := os.Executable()
	...
	data := struct {
		...
		Label:   "com.automationhub.master",
		Program: fmt.Sprintf("%s/hub", exPath),
		ProgramArguments: []string{
			"master",
			"--config",
			configPath,
	...
	f, err := os.OpenFile(plistPath, os.O_CREATE|os.O_TRUNC|os.O_RDWR, 0600)
	defer f.Close()
	t := template.Must(template.New("launchdConfig").Parse(macLaunchAgentTemplate()))
	err = t.Execute(f, data)
	if err != nil {
		fmt.Printf("Template generation failed: %s\n", err)
	}
	return plistPath
}

func macLaunchAgentTemplate() string {
	return `
 ...
 <plist version='1.0'>
   <dict>
     ...
     <array>
      ...
      {{range $val := .ProgramArguments}}<string>{{$val}}</string>
      {{end}}
     </array>

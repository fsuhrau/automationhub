...
func (d *Device) IsAppInstalled(params *app.Parameter) (bool, error) {
	appDir := filepath.Join(AppInstallPath, params.Hash, params.Name)
...
func (d *Device) InstallApp(params *app.Parameter) error {
	appDir := filepath.Join(AppInstallPath, params.Hash)
	_, err := app.Unzip(params.AppPath, appDir)
	if err != nil {
		return err
	}

	executable := filepath.Join(AppInstallPath, params.Hash, params.Name, params.LaunchActivity)
	return os.Chmod(executable, 0755)
}

func (d *Device) UninstallApp(params *app.Parameter) error {
	return os.RemoveAll(filepath.Join(AppInstallPath, params.Hash))
}

func (d *Device) StartApp(params *app.Parameter, sessionId string, nodeUrl string) error {
	appDir := filepath.Join(AppInstallPath, params.Hash, params.Name)
	executable := filepath.Join(appDir, params.LaunchActivity)
...
func (d *Device) StopApp(params *app.Parameter) error {
	executable := filepath.Base(params.LaunchActivity)
